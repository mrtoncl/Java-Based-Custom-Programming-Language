public class AddExpression extends ArithmeticExpression {
    ...
    AddExpression(Expression value1, Expression value2) {
        this.value1 = value1;
        this.value2 = value2;
    ...
    Expression execute() {
        if (value1.getValue() instanceof String || value2.getValue() instanceof String) {
            return new StringLiteral((value1.stringValue() + value2.stringValue()));
        } 
        else if (value1.getValue() instanceof Double) {
            if(value2.getValue() instanceof Double)
                return new DoubleLiteral((double) value1.getValue() + (double) value2.getValue());
            else
                return new DoubleLiteral((double) value1.getValue() + (int) value2.getValue());         //Checking the type of values and executing the expression.
        } 
        else if(value1.getValue() instanceof Integer){
            if(value2.getValue() instanceof Double)
